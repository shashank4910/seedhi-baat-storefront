...
(function () {
  ...
  function populateUI() {
    ...
    const singles = products.filter(p => p.kind === 'single' && p.id !== 'ideas-25-under-2k' && p.id !== 'ai-pro-guide');
    const singlePrice = singles.length ? singles[0].amount : 19900;
    bundleProducts.forEach((product) => {
      ...
      const save = fullPrice - product.amount;
      const savePct = Math.round((save / fullPrice) * 100);
      const saveLine = save > 0
        ? `<s>${formatPrice(fullPrice, product.currency)}</s> ${formatPrice(product.amount, product.currency)} <em class="bundle-save">save ${formatPrice(save, product.currency)} (${savePct}% off)</em>`
        : formatPrice(product.amount, product.currency);
      item.innerHTML = `
        <span class="badge">${esc(product.badge || count + ' books')}</span>
        <h3>${esc(product.name)}</h3>
        <p>${esc(product.description || '')}</p>
        <div class="bundle-meta">
          <div class="bundle-price"><strong>${saveLine}</strong><span>One-time · instant PDF</span></div>
          <button type="button" class="button bundle-select">Get the set</button>
        </div>
      `;
    ...
    bookProducts.forEach(product => {
      ...
      if (!SHOW_HINDI_BOOKS && book.language === 'Hindi') return;
